    ...
    return (   
        <div>
            {discountedCars.map((discountedcar,index) => ( 
                <div className={`carousel-item ${index===0&& 'active' ? "active" : ""}`}>
                    ...
                    <div className={`${style.carouselcaptioned}  d-md-block ${style.captionWrapper}`}>
                        <h5>{discountedcar.make} {discountedcar.model} {discountedcar.year}</h5>
                        <p className="text-danger" >{formatSum(discountedcar.discountedprice())}</p>
                    </div>
                </div>
            ))}
        </div>
